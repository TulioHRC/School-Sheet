...
def application():
    ...
    class MainApp():
        def __init__(self, master):
            ...
            Button(self.master, text="Novo :(", command=newLevel).grid(
                row=2, column=0, pady=5)
            Button(self.master, text="Deletar :)", command=delLevel).grid(
    ...
    class config(MainApp):
        ...
        def killSubjectsFrame(self):
            ...
            Label(self.killSubjects, text="Name: ").grid(
                row=0, column=0, pady=5, padx=5)
            self.namek = StringVar()
            self.nk = OptionMenu(self.killSubjects, self.namek,
                                *materias["name"].values)
    ...
    class delLevel(MainApp):
        def __init__(self):
            ...
            Label(self.newScreen, text="Matéria: ").grid(
                row=0, column=0, padx=2, pady=2)
            self.subject = StringVar()
            self.Options = OptionMenu(
                self.newScreen, self.subject, *subs_norm+subs_tec, command=self.changeL)
            self.Options.grid(row=0, column=1)

            Label(self.newScreen, text="Nome: ").grid(
                row=0, column=2, padx=2, pady=2)
            self.name = StringVar()
            self.n = OptionMenu(self.newScreen, self.name, "")
            self.n.grid(row=0, column=3)

            Button(self.newScreen, text="Deletar", command=self.delete).grid(
    ...
    class newLevel(MainApp):
        def __init__(self):
            ...
            Label(self.newScreen, text="Tipo: ").grid(
                row=1, column=0, padx=2, pady=2)
            self.type = StringVar()
            self.type.set('Medio')
            self.types = OptionMenu(self.newScreen, self.type,
                                    *["Medio", "Tecnico"], command=self.changeOps)
            ...
            self.subject = StringVar()
            self.Options = OptionMenu(
                self.newScreen, self.subject, *subs_norm)
            self.Options.grid(row=2, column=1)

            Label(self.newScreen, text="Valor: ").grid(
                row=0, column=2, padx=2, pady=2)
            self.val = Entry(self.newScreen)
            self.val.grid(row=0, column=3, padx=2, pady=2)

            Label(self.newScreen, text="Grupo: ").grid(
            ...
            Label(self.newScreen, text="Prazo: ").grid(
            ...
            Button(self.newScreen, text="Criar", command=self.create,
